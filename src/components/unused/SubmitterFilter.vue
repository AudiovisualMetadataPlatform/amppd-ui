<template>
  <div class="dropdown">
    <button
      ...
      :class="{
        show: workflowDashboard.filtersEnabled.submitterFilter === true,
      }"
      type="button"
      id="dropdownMenuButton1"
      data-toggle="dropdown"
      aria-haspopup="true"
      aria-expanded="workflowDashboard.filtersEnabled.submitterFilter ? 'true' : 'false'"
    ...
    >
      <form class="main-dropdown">
        <div class="container-fluid">
          <label for="colFormLabelSearch" class=" bold form-label">Submitter Name</label>
          <div class="input-group mb-3">
            <typeahead
              :source="getSubmitters"
              filter-key="submitter"
      ...
      <div class="container">
        <div class="card p-2">
          ...
          <div class="selected-submitter-cards">
            <!--loop through all the selected here -->
            <button
              class="btn btn-outline submitter"
              v-for="(submitter, index) in workflowDashboard.searchQuery
                .filterBySubmitters"
              v-bind:submitter="submitter"
              v-bind:index="index"
              v-bind:key="submitter.id"
            >
              <svg
                class="user-remove"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 50 50"
                @click="removeSubmitter(index)"
              >
                <path
...
export default {
  ...
  methods: {
    setFilterFlags() {
      this.workflowDashboard.filtersEnabled.submitterFilter = !this
    ...
    addSubmitter(submitter) {
      ...
      if (this.selectedSubmitters.length > 0) {
        ...
        else console.log("submitter already exists" + this.selectedSubmitters);
      } else this.selectedSubmitters.push(submitter);
    },
  ...
  watch: {
    selectedSubmitters: function() {
      this.selectedSubmitters.length > 0
        ? (this.workflowDashboard.filtersEnabled.submitterFilter = true)
        : (this.workflowDashboard.filtersEnabled.submitterFilter = false);
    },
  },
};

<template>
	<div class="dropdown" >
		<button class="btn btn-info dropdown-toggle" :class="{ 'show' : workflowDashboard.filtersEnabled.collectionFilter === true }" type="button" id="dropdownMenuButton1" 
			data-toggle="dropdown" aria-haspopup="true" aria-expanded="workflowDashboard.filtersEnabled.collectionFilter ? 'true' : 'false'" v-on:click="setFilterFlags">
			Collection
		</button>
		<div v-click-outside="closeFilter" class="dropdown-menu compact-form" :class="{ 'show' : workflowDashboard.filtersEnabled.collectionFilter === true }" aria-labelledby="dropdownMenuButton" 
			...
			<form class="main-dropdown">
				<div class="container-fluid">
					<label for="colFormLabelSearch" class=" bold">Collection Name</label>
					<div class="input-group mb-3">
						<typeahead :source="getCollections" filter-key="collection" filter-type="contains" :start-at="1" @selection="addCollection"
							id="colFormLabelSearch" class="form-control bootstrap-typeahead" placeholder="Search Collection Name"/>
			...
			<div class="container">
				<div class="card p-2">
					...
					<div class="selected-workflow-cards">  <!--loop through all the selected here -->
						<button class="btn btn-outline workflow" v-for = "(collection,index) in workflowDashboard.searchQuery.filterByCollections" v-bind:collection="collection" v-bind:index="index" v-bind:key="collection.id">
							<svg class="user-remove" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" @click="removeCollection(index)">
...
export default {
  ...
  computed:{
		...
		collections: sync("workflowDashboard.searchResult.filters.collections"),
		getCollections(){
			if(!this.collections) return [];
			return this.collections;
		},
		selectedCollectionss(){
			return this.workflowDashboard.searchQuery.filterByCollections;
		}
  },
  
  methods:{
	setFilterFlags(){
		this.workflowDashboard.filtersEnabled.collectionFilter = !this.workflowDashboard.filtersEnabled.collectionFilter;
		if(this.workflowDashboard.filtersEnabled.collectionFilter)
		{
		...
		addCollection(collection) {
			if (this.selectedCollections.length >0) {
				if (this.selectedCollections.indexOf(collection) == -1) {
					this.selectedCollections.push(collection);
				}
				else {
					console.log("Collection already exists" + this.selectedCollections);
				}
			}
			else {
				this.selectedCollections.push(collection);
		...
		removeCollection(index) {
			console.log("removing collection at:" + index);
			var removed = this.selectedCollections.splice(index,1);
			console.log("selected collection are: " + this.selectedCollections + ", and removed element is: " + removed);
		},
		closeFilter(){
  ...
  watch: {
		selectedCollections: function() {
			this.selectedCollections.length>0 ? this.workflowDashboard.filtersEnabled.collectionFilter = true : this.workflowDashboard.filtersEnabled.collectionFilter = false}
  }, 

            ...
            >
              <div class="card-body">
                ...
                <ul class="list-unstyled file-list">
                  <li
                    v-for="(primaryfile, indexPf) in bundle.primaryfiles"
                    v-bind:key="indexPf"
...
export default {
  ...
  methods: {
    ...
    addAllFiles(bundle) {
      ...
      pfs.forEach((pf) => {
        if (!this.selectedFiles.has(pf.id)) {
          ...
          this.workflowSubmission.updateSelectedFiles =
            this.updateSelectedFiles + 1;
          console.log(
            "Added selected primaryfile " + pf.id + " from bundle " + bundle.id
          );
        } else {
          console.log(
            "Primaryfile " +
              pf.id +
              " already selected from bundle " +
              bundle.id
          );
        }
      });
    ...
    setupBundleError() {
      ...
      this.responseText =
        "Error retrieving bundles: a system error has occured, please try again later.";
    },
  },

...
export default {
  ...
  methods:{
    ...
    createBundle(){
      ...
      await axios.post(process.env.VUE_APP_AMP_URL + '/bundles', newBundle)
        .then(response => {
      ...
      submitWorkflow(){
        await axios.post(process.env.VUE_APP_AMP_URL + '/jobs/bundle?workflowId=' + selectedWorkflow.id + '&bundleId=' + this.bundle.id + '&parameters={}')
        .then(response => {

<template>
  <div>
    ...
    <ul
      v-show="isOpen"
      class="autocomplete-results w-100"
  ...
  methods: {
    onChange() {
      ...
      if (self.items.length > 0) {
        ...
        console.log(
          "not calling filter results as items.length is:" + self.items.length
        );
    ...
    filterResults() {
      ...
      if (self.filterType == "starts") {
        return self.items.filter((item) => {
          return (
            item && item.toLowerCase().startsWith(self.query.toLowerCase())
          );
        });

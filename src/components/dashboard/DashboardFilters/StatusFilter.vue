		...
		<div class="dropdown-menu compact-form" :class="{ 'show' : visible === true }" aria-labelledby="dropdownMenuButton" 
			...
			<div class="container">
				<div class="card p-2">
					...
					<div class="selected-status-cards">  <!--loop through all the selected here -->
						<button class="btn btn-outline status" v-for = "(status,index) in workflowDashboard.searchQuery.filterByStatuses" v-bind:status="status" v-bind:index="index" v-bind:key="status.id">
							<svg class="user-remove" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" @click="removeStatus(index)">
